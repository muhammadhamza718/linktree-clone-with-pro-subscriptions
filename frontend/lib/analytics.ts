...
import { AnalyticsEventExtended } from '../types';
...
export async function trackAnalyticsEvent(data: AnalyticsTrackData): Promise<string> {
  // In a real implementation, this would add the event to a queue for async processing
  // For now, we'll save directly to the database

  try {
    const event = await prisma.analyticsEventExtended.create({
      data: {
    ...
    // Don't throw error to prevent impacting user experience
    return '';
  }
}
...
export async function getAnalyticsDashboard(
  ...
  try {
    ...
    const [totalViews, totalClicks, demographics, referrers, heatmaps] = await Promise.all([
      // Total profile views
      prisma.analyticsEventExtended.count({
        where: {
          profileId,
      ...
      prisma.analyticsEventExtended.groupBy({
    ...
    // Process device data
    const deviceCounts = demographics.reduce((acc: any, item) => {
      if (item.deviceType) {
        acc[item.deviceType] = (acc[item.deviceType] || 0) + item._count.country;
      }
      return acc;
    ...
    // Process heatmaps
    const linkHeatmaps = await Promise.all(heatmaps.map(async (heatmap) => {
    ...
    return {
      ...
      heatmaps: linkHeatmaps.filter(Boolean).sort((a, b) => b.clicks - a.clicks).slice(0, 10), // Top 10 links
    };
  } catch (error) {
...
export async function exportAnalyticsData(
  ...
  try {
    ...
    switch (dataType) {
      case 'clicks':
        data = await prisma.analyticsEventExtended.findMany({
          where: {
            profileId,
            eventType: 'link_click',
            timestamp: {
      ...
      case 'views':
        ...
        break;
      case 'referrers':
        data = await prisma.analyticsEventExtended.groupBy({
          by: ['referrer'],
          where: {
            profileId,
            eventType: 'profile_view',
